...
in_func = False
for line in lines:
    if line.startswith('@router.callback_query(F.data.startswith("user_reviews_carousel:"))'):
        in_func = True
        new_lines.append(line)
        continue
    if in_func and line.startswith('def register_handlers(dp: Dispatcher) -> None:'):
        in_func = False
    
    if not in_func:
        new_lines.append(line)

replacement = """async def handle_user_reviews_carousel(
...
idx = new_lines.index('@router.callback_query(F.data.startswith("user_reviews_carousel:"))\n')
new_lines.insert(idx + 1, replacement)
