...
## Дата: 2026-04-08
### Изменения:
- **Локализация админ-панели (Конфигурация бота):**
    - В `app/services/system_settings_service.py` добавлен словарь `SETTING_LABELS` для автоматической замены технических ключей (напр. `REFERRAL_...`) на понятные русские названия.
    - Обновлено свойство `display_name` в `SettingDefinition` для приоритетного использования локализованных названий.
    - В `app/handlers/admin/bot_configuration.py` переведены все кнопки навигации («Назад», «Сброс», «К разделам»), заголовки и инструкции по редактированию.
    - Улучшены всплывающие уведомления (callback answers) и сообщения об успешном сохранении.
    - Добавлены развернутые описания для системы **Конкурсов рефералов**, объясняющие механику и порядок запуска.
    - Расширена система подсказок `SETTING_HINTS` для ключевых разделов (Рефералы, Подарки, Система).
- **Оптимизация Premium-эмодзи (UX/UI):**
